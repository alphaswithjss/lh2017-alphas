...
int main() {
   ...
   for (int i=1; i<=gluons_beta0->GetNbinsX(); i++){
      ...
      gluons_beta0->SetBinContent(i, result(e2,myCA,myBg,zcut,0.,0.10,500.0,0.000001));
      quarks_beta0->SetBinContent(i, result(e2,myCF,myBq,zcut,0.,0.10,500.0,0.000001)); 

      gluons_beta1->SetBinContent(i, result(e2,myCA,myBg,zcut,1.,0.10,500.0,0.000001));
      quarks_beta1->SetBinContent(i, result(e2,myCF,myBq,zcut,1.,0.10,500.0,0.000001));         
   }
   ...
   l.SetNDC();
   l.SetTextColor(1);
   l.DrawLatex(0.2,0.85,"#bf{#smyCAle[0.5]{Softdrop @ MLL, 1402.2657}}"); 
   l.DrawLatex(0.2,0.8,"#bf{#smyCAle[0.5]{p_{T} = 500 GeV, z_{cut} = 0.1}}");    
  ...
  weights_SD(pt, R, zcut, beta, alphasMZ, muR, muC, mufr,
   ...
   //test radiator for a quark (use Ci=myCF, and Bi=myBq)
   printf("Test Radiator Quark = %f\n", Rad(0.0001,myCF,myBq,beta0,0.12,0.1,0.0));

   //test Sudakov for a quark (use Ci=myCF, and Bi=myBq)
   printf("Test Sudakov Quark = %f\n", sudakov(0.0001,myCF,myBq,0.1,0.0,0.12,500));

   //test diff Sudakov for a quark (use Ci=myCF, and Bi=myBq)
   printf("Test diff Sudakov Quark = %f\n", diffsudakov(0.0001,myCF,myBq,0.1,0.0,0.12,500,0.000001));

   //myCAll it for a quark (use Ci=myCF, and Bi=myBq)
   printf("Test Value Quark = %f\n", result(0.0001,myCF,myBq,0.1,0.0,0.12,500.0,0.000001));

   //test diff Sudakov for a gluon (use Ci=myCA, and Bi=myBg)
   printf("Test diff Sudakov Gluon = %f\n", diffsudakov(0.0001,myCA,myBg,0.1,0.0,0.12,500,0.000001));

   //myCAll it for a gluon (use myCA, and myBg)
   printf("Test Value Gluon = %f\n", result(0.0001,myCA,myBg,0.1,0.0,0.12,500.0,0.000001));
