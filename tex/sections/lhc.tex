...
\end{center}
\caption{\jdt{We should decide whether or not it is important to have the plot in the same direction as \Fig{fig:shape_function}.}  The quark and gluon templates, computed at NLL~\cite{Marzani:2017mva,Marzani:2017kqd} for $\alpha=2$ (left column) and 
...
%
These distributions are a superposition of the quark and gluon distributions and depend only on $\alpha_s$ \jdt{It also depends on $f_g$, right?}.
%
...
%combination23451324
\end{center}
\caption{\jdt{Shouldn't the legend say "Known Quark" and "Known Gluon"?}  Marginalizing the $f_g,\alpha_s$ fit over the gluon fraction for a fit that combines the four observables from \Fig{fig:templates}.  The unknown combined mixture uses two samples with $f_g=20\%$ and $f_g=80\%$.}
\label{fig:combo}
\end{figure}
...
\end{center}
\caption{\jdt{Isn't the region we are fitting to have an upper bound, too?  We should show that as well}  The fractional $\ecf{2}{2}$ distribution determined from dijet events simulated with \pythia plus \textsc{Delphes}.  Arrows indicate the mass at given values of the two-point correlator.  The upper bound of the resummation regime is indicated by a dashed line.  Larger masses are to the left.}
