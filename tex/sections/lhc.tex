...
\caption{The quark and gluon templates, computed at
  ...
  right and the fixed-order regime is on the left. \gs{The axes labels
  are not right}}
\label{fig:templates}
\end{figure}
...
\caption{Left: the probability of the minimized $\chi^2$ (assuming
  ...
  from the left plot ($f_g\in\{0.2,0.8\}$). \gs{The legend on the
...
from \pythia.x. \gs{Do not forget to fill in Delphes and Pythia
  versions.} The setup uses a CMS-like detector with jets built from
...
resolution.  The resolution is smallest at low mass \gs{at high mass?}
