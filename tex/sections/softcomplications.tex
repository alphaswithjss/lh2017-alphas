...
\end{align}
%
For $\zcut p_T R \gg \lamqcd$, we see that the grooming significantly suppresses the scale of the NP physics, extending the range of perturbative validity by a factor of $({\lamqcd} / {\zcut p_TR})^{1/(1+\beta)}$.
%
...
\end{align}
with $f_q=1$, $f_{\bar q}=-1$, $f_g=0$, and $J_{\text{SD}}$ are the parton-level constituents of the groomed jet.
%
The grooming procedure makes this definition IRC safe (see related jet flavor approach in \Ref{Banfi:2006hf}).
%
...
%
Jet shapes are particularly sensitive to pileup; for example, the jet mass scales as $\mathcal{O}(A^2)$~\cite{Salam:2009jx} for the jet catchment area $A$~\cite{Cacciari:2008gn} (whereas the jet $p_\text{T}$ scales linearly with $A$).
%
