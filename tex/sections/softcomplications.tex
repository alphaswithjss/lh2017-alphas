...
\item Improved Detector Resolution \gs{``and reduced pileup sensitivity''}
\end{itemize}
...
the interest in the measurement of the groomed jet mass \gs{``of the
  groomed jet observables''}, and the possibility of performing a
...
shapes is the issue of non-perturbative (NP) corrections, which cannot
...
operators cannot be calculated, they can be modeled using so called
shape functions
...
is highly correlated with $\alpha_s$, see
\Fig{fig:correlation_firstmoment} introducing uncertainties into the
extraction. Furthermore, there is disagreement over whether the use of
a single shape function moment is applicable in the region where
$\alpha_s$ fits are performed~\cite{}. Therefore, observables with
...
%
\gs{One should also mention the dispersive/renormalon approach from,
  e.g. hep-ph/9504219,  hep-ph/9512336, used also in arXiv:0706.3157
  for jets and in arXiv:1210.6945 for $\alpha_S$ extraction.}
...
\end{align}
\gs{I'd write this with $Q\to p_tR$. The above expression corresponds to a $1/(p_tR^\alpha)$ normalisation in the
  definition of $e_\alpha$. Also, for $\alpha<1$, it is replaced by $(\lamqcd/(p_tR))^\alpha$.} 
...
studies. \gs{What about -ve $\beta$. We could add sth like ``Negative
  $\beta$ could also be interesting to study although some specific
  attention would be needed in the vicinity of the endpoint of the
  dstribbution ($e+\alpha=c_{\text{cut}}^{-\alpha\beta}$ at LL).''.}
...
\gs{This should include an explicit Ref to the mMDT paper which was
  the first to show that this was (i) calculable and (ii) simple.}
...
\end{itemize}
The jet mass was calculated in \cite{Jouttenus:2013hs} \gs{also a
  calculation in arXiv:1207.1640, with no veto.} for $H+$ jet with a
...
\cite{Frye:2016aiz,Marzani:2017kqd,Marzani:2017mva} \gs{Our
  calculation was for dijets, so I'd say ``for dijets and
  $V$+jets''.}.  Ideally this would be done using $2\to 3$ matrix
