...
while 1:
    # file header
    lineq = read_until_matching(fileq, re.compile("^BEGIN YODA_HISTO1D /MC_LHJSS_Zjet|^# BEGIN YODA_HISTO1D /MC_LHJSS_Zjet"));
    lineg = read_until_matching(fileg, re.compile("^BEGIN YODA_HISTO1D /MC_LHJSS_Zjet|^# BEGIN YODA_HISTO1D /MC_LHJSS_Zjet"));
    fileo.write(lineq);
    ...
    #fileo.write(lineq)
    fileo.write(lineq)
    label=lineq.replace("Path=/MC_LHJSS_Zjet_EE/","").rstrip("\n")
