...
void RooUnfoldExample(){
  ...
  RooUnfoldBayes   unfold_hold (&myresponse, Reco, 4);
  ...
  for (int i=1; i<=Response->GetNbinsX(); i++){
      TH1D* hold = new TH1D("hold","hold",bins2.size()-1,&bins2[0]);
      for (int j=1; j<=Response->GetNbinsY(); j++){
        hold->SetBinContent(j,Response->GetBinContent(j,i));
      }
      hold->Scale(1./hold->Integral());
      for (int j=1; j<=Response->GetNbinsY(); j++){
        Response->SetBinContent(j,i,hold->GetBinContent(j));
      }      
  }
  gPad->SetRightMargin(0.2);
  gStyle->SetPaintTextFormat("4.1f");
  Response->SetTitle("");
  Response->GetXaxis()->SetTitle("Detector-level m/p_{T}");
  Response->GetYaxis()->SetTitle("Particle-level m/p_{T}");
  Response->GetZaxis()->SetTitle("Pr(Detector-level | Particle-level)");
  Response->SetLabelFont(42);
  Response->SetTitleFont(42);
  Response->GetZaxis()->SetLabelFont(42);
  Response->GetZaxis()->SetTitleFont(42);  
  Response->GetYaxis()->SetLabelFont(42);
  Response->GetYaxis()->SetTitleFont(42);  
  Response->GetYaxis()->SetNdivisions(505);
  Response->GetXaxis()->SetTitleOffset(1.4);
  Response->GetYaxis()->SetTitleOffset(1.6);  
  Response->GetZaxis()->SetTitleOffset(1.6);  
  Response->GetZaxis()->SetRangeUser(0,1);
  Response->Draw("colz");
  Response->SetMarkerSize(1);
  Response->Draw("textsame");
  c1->Print("plots/ResponseMatrix.pdf");
  gPad->SetRightMargin(0.05);
  ...
  myout2->SetTitle("");
  myout2->GetXaxis()->SetTitle("m/p_{T}");
  myout2->GetYaxis()->SetTitle("Normalized to unity");
  myout2->SetLabelFont(42);
  myout2->SetTitleFont(42);
  myout2->GetYaxis()->SetLabelFont(42);
  myout2->GetYaxis()->SetTitleFont(42);  
  myout2->GetYaxis()->SetNdivisions(505);
  myout2->GetXaxis()->SetTitleOffset(1.4);
  myout2->GetYaxis()->SetTitleOffset(1.6);  
  myout2->SetMarkerSize(1);
  myout2->SetMarkerStyle(20); 
  myout2->GetYaxis()->SetRangeUser(1,myout2->GetMaximum()*100);
  myout2->Draw("e1p");
  double min_val = 9999.;
  double Min_as = -1;
  for (double as = 0.05; as<0.2; as+=0.001){
   ...
   l.DrawLatex(0.2,0.8,"#bf{#scale[0.5]{p_{T} = 500 GeV, z_{cut} = 0.1, gluons only}}");     
   c1->Print("plots/example_fit.pdf");

  TH1F* myJMS = new TH1F("","",21,0.945,1.055);
  for (double JMS = 0.95; JMS < 1.06; JMS+=0.005){
   ...
   myJMS->SetBinContent(myJMS->FindBin(JMS),fabs(min_as-Min_as)/Min_as);
  }

  gPad->SetLogy(0);
  gPad->SetLogx(0);
  myJMS->SetLabelFont(42);
  myJMS->SetTitleFont(42);
  myJMS->GetYaxis()->SetLabelFont(42);
  myJMS->GetYaxis()->SetTitleFont(42);  
  myJMS->GetYaxis()->SetRangeUser(0,0.2);
  myJMS->GetYaxis()->SetNdivisions(505);
  myJMS->GetXaxis()->SetTitleOffset(1.4);
  myJMS->GetYaxis()->SetTitleOffset(1.6);
  myJMS->GetXaxis()->SetTitle("Jet Mass Scale");
  myJMS->GetYaxis()->SetTitle("#alpha_{s} uncertainty");    
  myJMS->Draw();

  l.DrawLatex(0.2,0.85,"#bf{#scale[0.5]{Softdrop @ MLL, A. Larkoski et al. 1402.2657}}"); 
  l.DrawLatex(0.2,0.8,"#bf{#scale[0.5]{p_{T} = 500 GeV, z_{cut} = 0.1, quark frac = "+TString::Format("%0.2f",q_frac)+"}}");   

  c1->Print("plots/JMS_qfrac"+TString::Format("%1.0f",q_frac*100)+".pdf");

  TH1F* myJMR = new TH1F("","",41,0.79,1.21);
  for (double JMS = 0.8; JMS < 1.23; JMS+=0.01){
    TH1D* Reco2 = new TH1D("Reco","Reco",bins2.size()-1,&bins2[0]);
    for (int ntoy = 0; ntoy < ntoys; ntoy++){
   ...
   myJMR->SetBinContent(myJMR->FindBin(JMS),fabs(min_as-Min_as)/Min_as);
  }

  gPad->SetLogy(0);
  gPad->SetLogx(0);
  myJMR->SetLabelFont(42);
  myJMR->SetTitleFont(42);
  myJMR->GetYaxis()->SetLabelFont(42);
  myJMR->GetYaxis()->SetTitleFont(42);  
  myJMR->GetYaxis()->SetRangeUser(0,0.2);
  myJMR->GetYaxis()->SetNdivisions(505);
  myJMR->GetXaxis()->SetTitleOffset(1.4);
  myJMR->GetYaxis()->SetTitleOffset(1.6);
  myJMR->GetXaxis()->SetTitle("Jet Mass Resolution");
  myJMR->GetYaxis()->SetTitle("#alpha_{s} uncertainty");    
  myJMR->Draw();

  l.DrawLatex(0.2,0.85,"#bf{#scale[0.5]{Softdrop @ MLL, A. Larkoski et al. 1402.2657}}"); 
  l.DrawLatex(0.2,0.8,"#bf{#scale[0.5]{p_{T} = 500 GeV, z_{cut} = 0.1, quark frac = "+TString::Format("%0.2f",q_frac)+"}}");  

  c1->Print("plots/JMR_qfrac"+TString::Format("%1.0f",q_frac*100)+".pdf");
